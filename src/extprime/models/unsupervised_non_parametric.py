...
def analyze_clustering(algo_names, summary_dataset, env):
    ...
    for name in algo_names:
        ...
        # ADD THE F1-SCORE CODE HERE:
        results_df['purity'] = 1 - results_df['contamination']
        results_df['f1_score'] = 2 * (results_df['completeness'] * results_df['purity']) / (
                    results_df['completeness'] + results_df['purity'])
        print(f"Mean F1 Score for {name}: {results_df['f1_score'].mean():.3f}")
        print(f"Mean Completeness for {name}: {results_df['completeness'].mean():.3f}")
        print(f"Mean Contamination for {name}: {results_df['contamination'].mean():.3f}")
        print("---")
