...
def process_sequence(lines, seq_id, min_len, names, seqs, plasmids):
    ...
    if len(sequence) >= min_len:
        ...
        # excluding plasmid in the fragment building
            names.append(seq_id)
...
def run_fragment_builder(path, fragment_file, fragment_length, whole_genome, env, n=10):
    ...
    process_environmental_data(dataset_env, path, env_folder, env, fragment_length, whole_genome, n)
...
def process_environmental_data(dataset_env, path, env_folder, env_type, fragment_length, whole_genome, n=10):
    aux_dataset = []
    ...
    for index, row in dataset_env.iterrows():
        ...
        if seq_names == None or seqs == None:
            ...
            fragment = produce_fragment(seq_names, seqs, fragment_length, is_whole_genome=whole_genome, max_num_part=n)
...
def process_assembly(assembly_path, row, env_folder):
    ...
    # use this to filter the sequence based on the length, the default is 0, so all sequences will be included
    _min = 0
